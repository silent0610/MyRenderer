...
void Renderer::SetBuffer(uint8_t* buffer, const int x, const int y, const Vec4f& color) const
{
...
Renderer::Vertex& Renderer::VertexLerp(Vertex& vertex_p0, Vertex& vertex_p1, const float ratio)
{
	auto* vertex = new Vertex();
	vertex->position = vector_lerp(vertex_p0.position, vertex_p1.position, ratio);

	Varyings& context = vertex->context;
	Varyings& context_p0 = vertex_p0.context;
	Varyings& context_p1 = vertex_p1.context;

	//遍历 context_p0.varying_float 容器中的所有键。key 是每次迭代时访问的一个键。
...
/// @param y2 
/// @param color 
void Renderer::DrawLine(int x1, int y1, int x2, int y2, const Vec4f& color) const
{
	int x, y;
	if (x1 == x2 && y1 == y2) //重合
	{
...
void Renderer::DrawMesh()
{
	...
	*/
	...
	if (normal.z <= 0) return; //如果法线z分量小于0,则背对相机
...
void Renderer::RasterizeTriangle(Vertex* vertex[3])
{
	// 三角形屏幕空间中的外接矩形
	...
	for (int y = bounding_min.y; y <= bounding_max.y; y++)
	{
		for (int x = bounding_min.x; x <= bounding_max.x; x++)
		{
			...
			// 当不属于上边缘或者左边缘的时候，将e与1进行比较，从而跳过e=0的情况
			//
			...
			float bc_p0 = e0 * bc_denominator;  //不正确的重心坐标, 不能插值除深度外的
			float bc_p1 = e1 * bc_denominator;
			float bc_p2 = e2 * bc_denominator;

			// 计算透视正确的重心坐标
			float bc_correct_denominator =
				e0 * edge_equation_[0].w_reciprocal +   //这里的1/w是 的w 是三角形三个顶点的w
